...
def AddAssertion(info):
  ...
  info.script.AppendExtra('ui_print("and then flash MoKee again");')
...
def ReplaceApnList(info):
  ...
  info.script.AppendExtra('run_program("/sbin/sh", "-c", "mv /system/etc/cdma/eri-boost.xml /system/etc/eri.xml");')
  info.script.AppendExtra('endif;')
  ...
  info.script.AppendExtra('run_program("/sbin/sh", "-c", "mv /system/etc/cdma/eri-frdm.xml /system/etc/eri.xml");')
  info.script.AppendExtra('endif;')
  info.script.AppendExtra('if getprop("ro.boot.fsg-id") == "ringplus" || getprop("ro.boot.fsg-id") == "rngplus" then')
  info.script.AppendExtra('run_program("/sbin/sh", "-c", "mv /system/etc/cdma/apns-conf-rngplus.xml /system/etc/apns-conf.xml");')
  info.script.AppendExtra('run_program("/sbin/sh", "-c", "mv /system/etc/cdma/eri-rngplus.xml /system/etc/eri.xml");')
  info.script.AppendExtra('endif;')
  ...
  info.script.AppendExtra('run_program("/sbin/sh", "-c", "mv /system/etc/cdma/eri-virgin.xml /system/etc/eri.xml");')
  info.script.AppendExtra('endif;')
  info.script.AppendExtra('endif;')
